...
export class MsalAzureCodeGrant extends MsalAzureAuth {
    ...
    constructor(
        ...
        this.pkceCodes = {
            nonce: "",
            challengeMethod: "S256", // Use SHA256 as the challenge method
    ...
    protected async login(tenant: ITenant): Promise<{
        ...
        console.info(`Authority URL set to: ${authority}`);
        ...
        try {
            ...
            console.error("MSAL: Error requesting auth code", e);
        ...
        if (!result) {
            console.error("Failed to acquireTokenByCode");
            console.error(
    ...
    private async addServerListeners(
        ...
        server.on("/signin", (req, reqUrl, res) => {
            ...
            if (receivedNonce !== nonce) {
                ...
                res.write("Authentication failed due to a nonce mismatch, please close Azure Data Studio and try again.");
                res.end();
        ...
        return new Promise<string>((resolve, reject) => {
            server.on("/redirect", (req, reqUrl, res) => {
                ...
                if (split.length !== 2) {
                    res.writeHead(400, { "content-type": "text/html" });
                    res.write("Authentication failed due to a state mismatch, please close ADS and try again.");
                    res.end();
